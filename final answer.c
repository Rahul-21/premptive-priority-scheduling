...
struct data
{
int at,st,ct,tat,wt,p;
char k[20];
float ntat;
}temp;
main()
{
printf("\npremptive priority behaving as round robin");
...
for(i=0;i<n;i++)
{
printf("enter the details of process %d,at,st\n",i);
...
printf("gantt chart sequence \n");
for(i=0;i<8;i++)
{
	printf("process -%d  -->",queue[i]);
}
...
for(i=0;i<n;i++)
{
printf("%d\t%d\t%d\t%d\t%d\t%f\n",a[i].at,a[i].st,a[i].ct,a[i].tat,a[i].wt,a[i].ntat);
}
printf("avgtat=%f\navgwt=%f\n",avgtat,avgwt);
}
